...
- Prefer `{x:Bind}` over `{Binding}` for better performance and compile-time validation.
    - Set `x:DataType` on templates to enable compile‑time validation for `{x:Bind}` compiled bindings.
    - Prefer functions in `{x:Bind}` for simple transformations instead of creating a new converter.
...
- The source language for resource files is English (United States). Add all new strings to the en-US resource file only. Other language resource files should be created by Crowdin.
...
<Button
    ...
    AccessKey="{x:Bind strings:KeyboardResources.PlayerPlayPauseKey}"
    AutomationProperties.Name="{x:Bind helpers:ItemLabelHelper.GetPlayPauseLabel(ViewModel.IsPlaying), Mode=OneWay}"
